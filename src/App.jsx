...
import { FaFacebook, FaInstagram, FaChevronLeft, FaChevronRight, FaClock, FaPlus, FaTimes, FaSearch, FaChevronRight as FaChevronRightIcon } from 'react-icons/fa';
...
export default function App() {
    ...
    }, [selectedAccountIds, currentDate, view, fetchAllPostsForSelectedAccounts]);
    ...
    const handleSelectAll = (filteredAccountIds) => {
        const currentSelection = new Set(selectedAccountIds);
        const allFilteredSelected = filteredAccountIds.every(id => currentSelection.has(id));

        if(allFilteredSelected && filteredAccountIds.length > 0) {
            filteredAccountIds.forEach(id => currentSelection.delete(id));
        } else {
            filteredAccountIds.forEach(id => currentSelection.add(id));
        }
...
const CollapsibleClientSelector = ({ accounts, selectedAccountIds, onSelectionChange, onSelectAll, isLoading, error }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-md">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center p-4">
                ...
                <FaChevronRightIcon className={`transition-transform ${isOpen ? 'rotate-90' : ''}`} />
            </button>
            {isOpen && (
                <div className="p-4 border-t">
                    ...
                    {isLoading ? <div className="flex justify-center p-4"><Spinner /></div> : filteredAccounts.length > 0 ? (
                        <>
                            <button onClick={() => onSelectAll(filteredAccounts.map(a => a.id))} className="mb-2 px-3 py-1 text-xs font-medium border border-gray-300 rounded-md hover:bg-gray-100">
                                Selecionar/Limpar filtrados
                            </button>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-60 overflow-y-auto">
                                {filteredAccounts.map(acc => (
                                    <label key={acc.id} className="flex items-center p-3 rounded-lg border hover:bg-gray-50 transition-colors cursor-pointer">
                                        <input type="checkbox" checked={selectedAccountIds.has(acc.id)} onChange={() => onSelectionChange(acc.id)} className="h-5 w-5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
